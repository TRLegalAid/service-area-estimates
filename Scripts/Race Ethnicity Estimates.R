...
Service_area_PRE_summary <- inSA3 %>%
  filter(variable == 'S1701_C02_013'| 
           variable == "S1701_C01_001" |
           variable == "S1701_C02_001" |
           variable == 'S1701_C02_014'| 
           variable == 'S1701_C02_015'| 
           variable == 'S1701_C02_016'| 
           variable == 'S1701_C02_017'| 
           variable == 'S1701_C02_018'| 
           variable == 'S1701_C02_019'| 
           variable == 'S1701_C02_020'| 
           variable == 'S1701_C02_021') %>%
  group_by(variable) %>%
  mutate(TOTAL = sum(estimate), MoError = moe_sum(moe, estimate)) %>%
  filter(GEOID == 48029) %>%
  select(-estimate, -moe, -NAME)

total_poverty_pop = 8036340
total_pop_under_100 = 1478400
